...
public class VkMirror {
    ...
    private void newReply(TdApi.Message message) {
        ...
        if(message.senderUserId == bot.getMyId()) return;
    ...
    public void sendMessage(VkMirrorChat chat, Message message) {
        if(!vkMirrorDao.isSyncedVk(chat, message.getMessageId())) {
            VkMirrorTelegram client = bot;
            if((message.getFlags() & Message.Flags.OUTBOX) > 0) {
                client = telegram;
            }

            VkMirrorTelegram finalClient = client;
            client.groupById(chat.getTelegramChannelId()).thenAccept(g -> {
                logger.info("Sending to chat {}", g.id);
                executor.submit(() -> {
                    synchronized (this) {
                        TdApi.Message tgMessage = finalClient.sendMessage(g.id, message.getText()).join();
                        logger.info("Message send {}", tgMessage);
